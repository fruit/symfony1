    ...
    public function process(ContainerBuilder $container)
    {
      ...
      if (!isset($config['logger']['class']))
      {
        throw new \InvalidArgumentException(
          sprintf('The "logger" class was not properly configured in file factories.yml')
        );
      }
      ...
      if (isset($mainLoggerParams['loggers']) && is_array($mainLoggerParams['loggers']))
      {
        foreach ($mainLoggerParams['loggers'] as $loggerName => $loggerAttr)
        {
          ...
          // Exclusion, sfWebDebugLogger depends on sfContext
          //if (is_subclass_of($loggerAttr['class'], 'sfWebDebugLogger') || $loggerAttr['class'] === 'sfWebDebugLogger') {
          //  $definition->addMethodCall('setContext', array(new Reference('sf_context')));
          //}
